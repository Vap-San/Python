        ...
        if len(nums_list) > 1:
            try:
                ...
                await call.message.edit_text(f"{nums}"
                                             f" = {check_list[0]}",
                                             reply_markup=choice)
                logging.info(f'Результат {nums} = {check_list[0]}')
            nums = ""
    else:
        logging.info(f'Пользователь не ввел значения')
        await call.message.edit_text("Введите значения", reply_markup=choice)
...
async def nums_choice(call: CallbackQuery, callback_data: dict):
    ...
    await call.message.edit_text(f"{nums}",
                                 reply_markup=choice)
    logging.info(f'Пользователь ввел {nums}')
...
async def stop_handler(message: Message):
    ...
    logging.info(
        f'{user_id=} {user_full_name=} {time.asctime()} - End work Bot')
    await message.reply(f"Bye, {user_name}")

    dp.stop_polling()
    logging.info(f'{time.asctime()} - The server has stopped')


@dp.message_handler()
async def echo(message: Message):
    logging.info('Не верный ввод пользователем')
